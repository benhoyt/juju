...
class RelationData:
    ...
    def _from_dict(cls, d: dict[str, Any]) -> RelationData:
        return cls(
            ...
            related_units={k: UnitRelationData._from_dict(v) for k, v in d['related-units'].items()} if 'related-units' in d else {},
        )
...
class UnitInfo:
    ...
    def _from_dict(cls, d: dict[str, Any]) -> UnitInfo:
        return cls(
            ...
            leader=d['leader'],
            life=d.get('life') or '',
            relation_info=[RelationData._from_dict(x) for x in d['relation-info']] if 'relation-info' in d else [],
