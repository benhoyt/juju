...
func GetFields() map[string][]FieldInfo {
	...
	getFields(reflect.TypeOf(common.ModelInfo{}), m, "ModelInfo", 1)
	return m
}
...
func getNiceName(s string) string {
	...
	if s == "model.ModelType" {
		return "string"
	}
	if s == "status.Status" {
		return "string"
	}
	return strings.ReplaceAll(strings.ReplaceAll(strings.ReplaceAll(s, "common.", ""), "*", ""), "storage.", "")
}
